...
### 6. `i18n.py` (Интернационализация)
Система переводов для многоязычного интерфейса (~1080 строк):
- **TRANSLATIONS**: Словарь переводов для 8 языков (ru, en, es, fr, pt, zh-CN, hi, ar) — ~3.5 млрд носителей
- **STORY_PROMPTS**: Промпты для генерации сказок на 3 языках (ru, en, es)
- **t(key, lang)**: Функция получения перевода по ключу с fallback на русский
- **get_translations(lang)**: Получение всех переводов для языка
- **get_genre_list(lang)**: Список жанров на указанном языке
- **get_age_ranges(lang)**: Возрастные группы на указанном языке
- **get_story_prompt(lang)**: Получение промпта для генерации сказки на указанном языке
- **Архитектура**: Централизованный словарь, легко расширяемый для новых языков
...
- **Лендинг** (`landing.py`): Включен (Landing 3.0). Отображается для всех неавторизованных посетителей (порт 8501).
- **Генератор** (`app.py`): Доступен после прохождения авторизации (или эмулированной авторизации).
