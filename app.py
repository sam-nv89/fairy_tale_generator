...
if 'current_story' in st.session_state:
    try:
        ...
        with toolbar_cols[1]:
            # Озвучка — placeholder для анимации загрузки
            voice_btn_placeholder = st.empty()
            voice_btn_text = f"🎧 {t('processing_audio', user_lang).split('...')[0].split('…')[0].strip().split(' ', 1)[-1] if False else ''}"
            # Используем i18n ключи для текста кнопки
            ...
            clicked = voice_btn_placeholder.button(voice_btn_text, key="toolbar_voice", use_container_width=True)
